...
write.table(d, "numerama.dadvsi.news.txt", row.names = FALSE)
...
setwd("~/Desktop/numerama.dadvsi.corpus/")

d = dir()

kw = sapply(d, function(x) {
  r <- "((rapport|commission|mission)*\\s*[A-ZÉ]+[a-zâäëéèîïöôç-]*\\s*(-|de la|à|d'|de l'|du|des)*)+"
  k <- unique(str_trim(unlist(str_extract_all(readLines(x)[-2], r))))
  s <- which(tolower(k) %in% sw)
  k[-s]
})

kw <- lapply(kw, function(x) gsub(" à|La |Le |Les| En | des| d'", "", x))
