...
extract <- function(x) {
	message(x)
	html = htmlParse(paste0("http://www.zdnet.fr/rechercher/", x, "/hadopi.htm"))
	html = xpathApply(html, "//div[@class='river']//a[contains(@href,'/actualites/')]/@href")
	html = unique(html)
}

d = lapply(1:48, extract)
d = unlist(d)

#
#

write.table(d, "zdnet.hadopi.news.txt", row.names = FALSE)

# ------
# scrape
# ------

d = read.table("zdnet.hadopi.news.txt", stringsAsFactors = FALSE, header = TRUE)

folder = "zdnet.hadopi.corpus"
dir.create(folder)
setwd(folder)

exceptions = c(280, 306) # quiz pages, deleted since; don't parse
for(i in 1:nrow(d)) {
	f = paste0(gsub("http://www.zdnet.fr/actualites/|\\.htm", "", d$x[i]), ".txt")
	if(!file.exists(f) & !i %in% exceptions) {
		message(i, "...: ", f)
		e = htmlParse(d$x[i])
		title = xpathApply(e, "//h1", xmlValue)[[1]]
		date = xpathApply(e, "//time", xmlValue)[[1]]
		txt = xpathApply(e, "//div[@class='storyBody']/p", xmlValue)
...
# save
sink("../zdnet.hadopi.keywords.txt")
freq
sink()
