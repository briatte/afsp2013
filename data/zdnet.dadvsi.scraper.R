...
extract <- function(x) {
	message(x)
	html = htmlParse(paste0("http://www.zdnet.fr/rechercher/", x, "/dadvsi.htm"))
	html = xpathApply(html, "//div[@class='river']//a[contains(@href,'/actualites/')]/@href")
	html = unique(html)
}

d = lapply(1:20, extract)
d = unlist(d)

#
#

write.table(d, "zdnet.dadvsi.news.txt", row.names = FALSE)

# ------
# scrape
# ------

d = read.table("zdnet.dadvsi.news.txt", stringsAsFactors = FALSE, header = TRUE)

folder = "zdnet.dadvsi.corpus"
dir.create(folder)
setwd(folder)
...
# save
sink("../zdnet.dadvsi.keywords.txt")
freq
sink()
